...
(function() {
  ...
  var isNode = typeof process !***REMOVED*** "undefined" && process && process.versions && process.versions.node;
  ...
  var userAgent = typeof navigator !***REMOVED*** "undefined" && navigator.userAgent;
  if (!userAgent) {
    return;
  }
...
// 1) We could be the application main() thread running in the main JS UI thread. (ENVIRONMENT_IS_WORKER ***REMOVED*** false and ENVIRONMENT_IS_PTHREAD ***REMOVED*** false)
// 2) We could be the application main() running directly in a worker. (ENVIRONMENT_IS_WORKER ***REMOVED*** true, ENVIRONMENT_IS_PTHREAD ***REMOVED*** false)
// 3) We could be an application pthread running in a worker. (ENVIRONMENT_IS_WORKER ***REMOVED*** true and ENVIRONMENT_IS_PTHREAD ***REMOVED*** true)
...
if (ENVIRONMENT_IS_NODE) {
  ...
  ENVIRONMENT_IS_PTHREAD = ENVIRONMENT_IS_WORKER && worker_threads["workerData"] ***REMOVED*** "em-pthread";
}
...
if (ENVIRONMENT_IS_NODE) {
  ...
  readAsync = async (filename, binary = true) => {
    ...
    assert(binary ? Buffer.isBuffer(ret) : typeof ret ***REMOVED*** "string");
    return ret;
  };
    ...
    readAsync = async url => {
      ...
      if (isFileURI(url)) {
        return new Promise((resolve, reject) => {
          ...
          xhr.responseType = "arraybuffer";
          xhr.onload = () => {
            if (xhr.status ***REMOVED*** 200 || (xhr.status ***REMOVED*** 0 && xhr.response)) {
...
if (ENVIRONMENT_IS_NODE) {
  ...
  var stringify = a => typeof a ***REMOVED*** "object" ? utils.inspect(a) : a;
...
// include: preamble.js
// ***REMOVED***= Preamble library stuff ***REMOVED***=
...
(() => {
  ...
  h16[0] = 25459;
  if (h8[0] !***REMOVED*** 115 || h8[1] !***REMOVED*** 99) abort("Runtime error: expected the system to be little-endian! (Run with -sSUPPORT_BIG_ENDIAN to bypass)");
})();
...
function isExportedByForceFilesystem(name) {
  return name ***REMOVED***= "FS_createPath" || name ***REMOVED***= "FS_createDataFile" || name ***REMOVED***= "FS_createPreloadedFile" || name ***REMOVED***= "FS_preloadFile" || name ***REMOVED***= "FS_unlink" || name ***REMOVED***= "addRunDependency" || // The old FS has some functionality that WasmFS lacks.
  name ***REMOVED***= "FS_createLazyFile" || name ***REMOVED***= "FS_createDevice" || name ***REMOVED***= "removeRunDependency";
}
...
if (ENVIRONMENT_IS_PTHREAD) {
  ...
  function handleMessage(e) {
    try {
      ...
      if (cmd ***REMOVED***= "load") {
        ...
        for (const handler of msgData.handlers) {
          ...
          if (!Module[handler] || Module[handler].proxy) {
            ...
            if (handler ***REMOVED*** "print") out = Module[handler];
            if (handler ***REMOVED*** "printErr") err = Module[handler];
          }
        }
      ...
      } else if (msgData.target ***REMOVED***= "setimmediate") {} else if (cmd ***REMOVED***= "checkMailbox") {
        if (initializedJS) {
          checkMailbox();
        }
      } else if (cmd) {
...
function preRun() {
  ...
  if (Module["preRun"]) {
    if (typeof Module["preRun"] ***REMOVED*** "function") Module["preRun"] = [ Module["preRun"] ];
...
function postRun() {
  ...
  // PThreads reuse the runtime from the main thread.
  if (Module["postRun"]) {
    if (typeof Module["postRun"] ***REMOVED*** "function") Module["postRun"] = [ Module["postRun"] ];
...
function getBinarySync(file) {
  if (file ***REMOVED*** wasmBinaryFile && wasmBinary) {
...
async function createWasm() {
  ...
  function receiveInstantiationResult(result) {
    ...
    assert(Module ***REMOVED***= trueModule, "the Module object should not be replaced during async compilation - perhaps the order of HTML elements is wrong?");
    trueModule = null;
...
var removeRunDependency = id => {
  ...
  if (runDependencies ***REMOVED*** 0) {
    if (runDependencyWatcher !***REMOVED*** null) {
...
var addRunDependency = id => {
  ...
  if (runDependencyWatcher ***REMOVED***= null && globalThis.setInterval) {
...
var ptrToString = ptr => {
  assert(typeof ptr ***REMOVED***= "number", `ptrToString expects a number, got ${typeof ptr}`);
...
var PThread = {
  ...
  loadWasmModuleToWorker: worker => new Promise(onFinishedLoading => {
    worker.onmessage = e => {
      ...
      if (cmd ***REMOVED***= "checkMailbox") {
        checkMailbox();
      } else if (cmd ***REMOVED***= "spawnThread") {
        spawnThread(d);
      } else if (cmd ***REMOVED***= "cleanupThread") {
        // cleanupThread needs to be run via callUserCallback since it calls
        // back into user code to free thread data. Without this it's possible
        // the unwind or ExitStatus exception could escape here.
        callUserCallback(() => cleanupThread(d.thread));
      } else if (cmd ***REMOVED***= "loaded") {
        worker.loaded = true;
      ...
      } else if (d.target ***REMOVED***= "setimmediate") {
        // Worker wants to postMessage() to itself to implement setImmediate()
        // emulation.
        worker.postMessage(d);
      } else if (cmd ***REMOVED***= "uncaughtException") {
      ...
      } else if (cmd ***REMOVED***= "callHandler") {
  ...
  getNewWorker() {
    if (PThread.unusedWorkers.length ***REMOVED*** 0) {
...
var getWasmTableEntry = funcPtr => {
  ...
  /** @suppress {checkTypes} */ assert(wasmTable.get(funcPtr) ***REMOVED*** func, "JavaScript-side Wasm function table mirror is out of date!");
  return func;
};
...
/**
  ...
  assert(typeof ptr ***REMOVED*** "number", `UTF8ToString expects a number (got ${typeof ptr})`);
  if (!ptr) return "";
...
var ___pthread_create_js = (pthread_ptr, attr, startRoutine, arg) => {
  ...
  // postMessage.
  if (ENVIRONMENT_IS_PTHREAD && (transferList.length ***REMOVED***= 0 || error)) {
...
var PATH = {
  isAbs: path => path.charAt(0) ***REMOVED***= "/",
  splitPath: filename => {
  ...
  normalize: path => {
    var isAbsolute = PATH.isAbs(path), trailingSlash = path.slice(-1) ***REMOVED***= "/";
    // Normalize the path
  ...
  relative: (from, to) => {
    ...
    function trim(arr) {
      ...
      for (;start < arr.length; start++) {
        if (arr[start] !***REMOVED*** "") break;
      }
      var end = arr.length - 1;
      for (;end >= 0; end--) {
        if (arr[end] !***REMOVED*** "") break;
      }
      if (start > end) return [];
    ...
    for (var i = 0; i < length; i++) {
      if (fromParts[i] !***REMOVED*** toParts[i]) {
        samePartsLength = i;
        break;
      }
    }
...
var stringToUTF8Array = (str, heap, outIdx, maxBytesToWrite) => {
  assert(typeof str ***REMOVED***= "string", `stringToUTF8Array expects a string (got ${typeof str})`);
...
var TTY = {
  ...
  stream_ops: {
    ...
    read(stream, buffer, offset, length, pos) {
      ...
      for (var i = 0; i < length; i++) {
        ...
        if (result ***REMOVED***= undefined && bytesRead ***REMOVED***= 0) {
          throw new FS.ErrnoError(6);
        }
        if (result ***REMOVED***= null || result ***REMOVED***= undefined) break;
        bytesRead++;
  ...
  default_tty_ops: {
    ...
    put_char(tty, val) {
      if (val ***REMOVED***= null || val ***REMOVED***= 10) {
...
var MEMFS = {
  ...
  resizeFileStorage(node, newSize) {
    if (node.usedBytes ***REMOVED*** newSize) return;
    if (newSize ***REMOVED*** 0) {
      node.contents = null;
  ...
  node_ops: {
    ...
    setattr(node, attr) {
      ...
      if (attr.size !***REMOVED*** undefined) {
  ...
  stream_ops: {
    ...
    write(stream, buffer, offset, length, position, canOwn) {
      ...
      if (buffer.subarray && (!node.contents || node.contents.subarray)) {
        ...
        if (canOwn) {
          assert(position ***REMOVED***= 0, "canOwn must imply no weird position inside the file");
          node.contents = buffer.subarray(offset, offset + length);
          node.usedBytes = length;
          return length;
        } else if (node.usedBytes ***REMOVED***= 0 && position ***REMOVED***= 0) {
    ...
    mmap(stream, length, position, prot, flags) {
      ...
      if (!(flags & 2) && contents && contents.buffer ***REMOVED***= HEAP8.buffer) {
...
var FS_modeStringToFlags = str => {
  ...
  if (typeof flags ***REMOVED*** "undefined") {
...
var IDBFS = {
  ...
  queuePersist: mount => {
    function onPersistComplete() {
      if (mount.idbPersistState ***REMOVED***= "again") startPersist(); else mount.idbPersistState = 0;
    }
    ...
    } else if (mount.idbPersistState ***REMOVED***= "idb") {
  ...
  reconcile: (src, dst, callback) => {
    ...
    var errored = false;
    var db = src.type ***REMOVED***= "remote" ? src.db : dst.db;
...
var FS_handledByPreloadPlugin = async (byteArray, fullname) => {
  ...
  for (var plugin of preloadPlugins) {
    if (plugin["canHandle"](fullname)) {
      assert(plugin["handle"].constructor.name ***REMOVED***= "AsyncFunction", "Filesystem plugin handlers must be async functions (See #24914)");
...
var FS = {
  ...
  ErrnoError: class extends Error {
    ...
    // we'll use the reliable test `err.name ***REMOVED*** "ErrnoError"` instead
    constructor(errno) {
      super(runtimeInitialized ? strError(errno) : "");
      this.errno = errno;
      for (var key in ERRNO_CODES) {
        if (ERRNO_CODES[key] ***REMOVED***= errno) {
          this.code = key;
          break;
        }
      }
  ...
  FSStream: class {
    ...
    get isRead() {
      return (this.flags & 2097155) !***REMOVED*** 1;
    }
    get isWrite() {
      return (this.flags & 2097155) !***REMOVED*** 0;
    }
    get isAppend() {
  ...
  FSNode: class {
    ...
    get read() {
      return (this.mode & this.readMode) ***REMOVED***= this.readMode;
    }
    set read(val) {
      val ? this.mode |= this.readMode : this.mode &= ~this.readMode;
    }
    get write() {
      return (this.mode & this.writeMode) ***REMOVED***= this.writeMode;
    }
    set write(val) {
  ...
  lookupPath(path, opts = {}) {
    ...
    linkloop: for (var nlinks = 0; nlinks < 40; nlinks++) {
      ...
      for (var i = 0; i < parts.length; i++) {
        var islast = (i ***REMOVED***= parts.length - 1);
        ...
        try {
          ...
          if ((e?.errno ***REMOVED***= 44) && islast && opts.noent_okay) {
            return {
              path: current_path
            };
          }
  ...
  getPath(node) {
    ...
    while (true) {
      if (FS.isRoot(node)) {
        ...
        return mount[mount.length - 1] !***REMOVED*** "/" ? `${mount}/${path}` : mount + path;
      }
  ...
  hashRemoveNode(node) {
    var hash = FS.hashName(node.parent.id, node.name);
    if (FS.nameTable[hash] ***REMOVED***= node) {
      FS.nameTable[hash] = node.name_next;
    } else {
      var current = FS.nameTable[hash];
      while (current) {
        if (current.name_next ***REMOVED***= node) {
  ...
  lookupNode(parent, name) {
    ...
    for (var node = FS.nameTable[hash]; node; node = node.name_next) {
      ...
      if (node.parent.id ***REMOVED***= parent.id && nodeName ***REMOVED***= name) {
        return node;
      }
    }
  ...
  createNode(parent, name, mode, rdev) {
    assert(typeof parent ***REMOVED*** "object");
  ...
  isFile(mode) {
    return (mode & 61440) ***REMOVED***= 32768;
  },
  isDir(mode) {
    return (mode & 61440) ***REMOVED***= 16384;
  },
  isLink(mode) {
    return (mode & 61440) ***REMOVED***= 40960;
  },
  isChrdev(mode) {
    return (mode & 61440) ***REMOVED***= 8192;
  },
  isBlkdev(mode) {
    return (mode & 61440) ***REMOVED***= 24576;
  },
  isFIFO(mode) {
    return (mode & 61440) ***REMOVED***= 4096;
  },
  isSocket(mode) {
    return (mode & 49152) ***REMOVED***= 49152;
  },
  ...
  mayDelete(dir, name, isdir) {
    ...
    if (isdir) {
      ...
      if (FS.isRoot(node) || FS.getPath(node) ***REMOVED***= FS.cwd()) {
        return 10;
      }
  ...
  mayOpen(node, flags) {
    ...
    if (FS.isDir(node.mode)) {
      // opening for write
      // TODO: check for O_SEARCH? (***REMOVED*** search for dir only)
      if (mode !***REMOVED*** "r" || (flags & (512 | 64))) {
        return 31;
      }
    }
  ...
  syncfs(populate, callback) {
    if (typeof populate ***REMOVED*** "function") {
      callback = populate;
      populate = false;
    }
    FS.syncFSRequests++;
  ...
  mkdev(path, mode, dev) {
    if (typeof dev ***REMOVED*** "undefined") {
      dev = mode;
      mode = 438;
    }
    mode |= 8192;
  ...
  rename(old_path, new_path) {
    ...
    if (old_dir.mount !***REMOVED*** new_dir.mount) {
  ...
  ftruncate(fd, len) {
    var stream = FS.getStreamChecked(fd);
    if (len < 0 || (stream.flags & 2097155) ***REMOVED***= 0) {
      throw new FS.ErrnoError(28);
    }
  ...
  open(path, flags, mode = 438) {
    ...
    flags = typeof flags ***REMOVED*** "string" ? FS_modeStringToFlags(flags) : flags;
    if ((flags & 64)) {
  ...
  read(stream, buffer, offset, length, position) {
    ...
    if ((stream.flags & 2097155) ***REMOVED***= 1) {
      throw new FS.ErrnoError(8);
    }
  ...
  write(stream, buffer, offset, length, position, canOwn) {
    ...
    if ((stream.flags & 2097155) ***REMOVED***= 0) {
      throw new FS.ErrnoError(8);
    }
    if (FS.isDir(stream.node.mode)) {
      throw new FS.ErrnoError(31);
  ...
  mmap(stream, length, position, prot, flags) {
    ...
    // the current process.
    if ((prot & 2) !***REMOVED*** 0 && (flags & 2) ***REMOVED***= 0 && (stream.flags & 2097155) !***REMOVED*** 2) {
  ...
  readFile(path, opts = {}) {
    ...
    if (opts.encoding !***REMOVED*** "utf8" && opts.encoding !***REMOVED*** "binary") {
  ...
  createStandardStreams(input, output, error) {
    ...
    assert(stdin.fd ***REMOVED***= 0, `invalid handle for stdin (${stdin.fd})`);
    assert(stdout.fd ***REMOVED***= 1, `invalid handle for stdout (${stdout.fd})`);
    assert(stderr.fd ***REMOVED***= 2, `invalid handle for stderr (${stderr.fd})`);
  },
  staticInit() {
  ...
  analyzePath(path, dontResolveLastLink) {
    ...
    try {
      ...
      ret.isRoot = lookup.path ***REMOVED***= "/";
    } catch (e) {
      ret.error = e.errno;
    }
    return ret;
  ...
  createPath(parent, path, canRead, canWrite) {
    parent = typeof parent ***REMOVED*** "string" ? parent : FS.getPath(parent);
  ...
  createFile(parent, name, properties, canRead, canWrite) {
    var path = PATH.join2(typeof parent ***REMOVED*** "string" ? parent : FS.getPath(parent), name);
  ...
  createLazyFile(parent, name, url, canRead, canWrite) {
    ...
    class LazyUint8Array {
      ...
      cacheLength() {
        ...
        xhr.send(null);
        if (!(xhr.status >= 200 && xhr.status < 300 || xhr.status ***REMOVED***= 304)) abort("Couldn't load " + url + ". Status: " + xhr.status);
        var datalength = Number(xhr.getResponseHeader("Content-length"));
        var header;
        var hasByteServing = (header = xhr.getResponseHeader("Accept-Ranges")) && header ***REMOVED***= "bytes";
        var usesGzip = (header = xhr.getResponseHeader("Content-Encoding")) && header ***REMOVED***= "gzip";
        ...
        var doXHR = (from, to) => {
          ...
          var xhr = new XMLHttpRequest;
          xhr.open("GET", url, false);
          if (datalength !***REMOVED*** chunkSize) xhr.setRequestHeader("Range", "bytes=" + from + "-" + to);
          ...
          xhr.send(null);
          if (!(xhr.status >= 200 && xhr.status < 300 || xhr.status ***REMOVED***= 304)) abort("Couldn't load " + url + ". Status: " + xhr.status);
          if (xhr.response !***REMOVED*** undefined) {
        ...
        lazyArray.setDataGetter(chunkNum => {
          ...
          if (typeof lazyArray.chunks[chunkNum] ***REMOVED*** "undefined") {
            lazyArray.chunks[chunkNum] = doXHR(start, end);
          }
          if (typeof lazyArray.chunks[chunkNum] ***REMOVED*** "undefined") abort("doXHR failed!");
...
function ___syscall_dup(fd) {
  ...
  try {
    ...
    if (typeof FS ***REMOVED*** "undefined" || !(e.name ***REMOVED***= "ErrnoError")) throw e;
    return -e.errno;
  }
}
...
function ___syscall_faccessat(dirfd, path, amode, flags) {
  ...
  try {
    ...
    assert(!flags || flags ***REMOVED*** 512);
...
var convertI32PairToI53Checked = (lo, hi) => {
  assert(lo ***REMOVED*** (lo >>> 0) || lo ***REMOVED*** (lo | 0));
  // lo should either be a i32 or a u32
  assert(hi ***REMOVED***= (hi | 0));
  // hi should be a i32
...
var stringToUTF8 = (str, outPtr, maxBytesToWrite) => {
  assert(typeof maxBytesToWrite ***REMOVED*** "number", "stringToUTF8(str, outPtr, maxBytesToWrite) is missing the third parameter that specifies the length of the output buffer!");
...
var __emval_decref = handle => {
  if (handle > 9 && 0 ***REMOVED***= --emval_handles[handle + 1]) {
    assert(emval_handles[handle] !***REMOVED*** undefined, `Decref for unallocated handle.`);
...
var Emval = {
  toValue: handle => {
    ...
    assert(handle ***REMOVED***= 2 || emval_handles[handle] !***REMOVED*** undefined && handle % 2 ***REMOVED***= 0, `invalid handle: ${handle}`);
...
var embindRepr = v => {
  ...
  var t = typeof v;
  if (t ***REMOVED***= "object" || t ***REMOVED***= "array" || t ***REMOVED***= "function") {
    return v.toString();
  } else {
    return "" + v;
  }
...
/** @suppress {globalThis} */ var __embind_register_integer = (primitiveType, name, size, minRange, maxRange) => {
  name = AsciiToString(name);
  const isUnsignedType = minRange ***REMOVED***= 0;
  ...
  registerType(primitiveType, {
    ...
    readValueFromPointer: integerReadValueFromPointer(name, size, minRange !***REMOVED*** 0),
    destructorFunction: null
  });
};
...
var __embind_register_std_string = (rawType, name) => {
  ...
  registerType(rawType, {
    ...
    toWireType(destructors, value) {
      ...
      var length;
      var valueIsOfTypeString = (typeof value ***REMOVED*** "string");
      // We accept `string` or array views with single byte elements
      if (!(valueIsOfTypeString || (ArrayBuffer.isView(value) && value.BYTES_PER_ELEMENT ***REMOVED*** 1))) {
...
var UTF16ToString = (ptr, maxBytesToRead, ignoreNul) => {
  assert(ptr % 2 ***REMOVED*** 0, "Pointer passed to UTF16ToString must be aligned to two bytes!");
  var idx = ((ptr) >> 1);
...
var stringToUTF16 = (str, outPtr, maxBytesToWrite) => {
  assert(outPtr % 2 ***REMOVED*** 0, "Pointer passed to stringToUTF16 must be aligned to two bytes!");
  assert(typeof maxBytesToWrite ***REMOVED*** "number", "stringToUTF16(str, outPtr, maxBytesToWrite) is missing the third parameter that specifies the length of the output buffer!");
...
var UTF32ToString = (ptr, maxBytesToRead, ignoreNul) => {
  assert(ptr % 4 ***REMOVED*** 0, "Pointer passed to UTF32ToString must be aligned to four bytes!");
  var str = "";
  var startIdx = ((ptr) >> 2);
...
var stringToUTF32 = (str, outPtr, maxBytesToWrite) => {
  assert(outPtr % 4 ***REMOVED*** 0, "Pointer passed to stringToUTF32 must be aligned to four bytes!");
  assert(typeof maxBytesToWrite ***REMOVED*** "number", "stringToUTF32(str, outPtr, maxBytesToWrite) is missing the third parameter that specifies the length of the output buffer!");
  // Backwards compatibility: if max bytes is not specified, assume unsafe unbounded write is allowed.
  maxBytesToWrite ??= 2147483647;
...
var __embind_register_std_wstring = (rawType, charSize, name) => {
  ...
  if (charSize ***REMOVED***= 2) {
    decodeString = UTF16ToString;
    encodeString = stringToUTF16;
    lengthBytesUTF = lengthBytesUTF16;
  } else {
    assert(charSize ***REMOVED***= 4, "only 2-byte and 4-byte strings are currently supported");
...
var handleException = e => {
  ...
  if (e instanceof ExitStatus || e ***REMOVED*** "unwind") {
    return EXITSTATUS;
  }
  checkStackCookie();
...
var __emscripten_notify_mailbox_postmessage = (targetThread, currThreadId) => {
  if (targetThread ***REMOVED*** currThreadId) {
...
var __emscripten_receive_on_main_thread_js = (funcIndex, emAsmAddr, callingThread, bufSize, args, ctx, ctxArgs) => {
  ...
  assert(func.length ***REMOVED*** proxiedJSCallArgs.length, "Call args mismatch in _emscripten_receive_on_main_thread_js");
...
var _emscripten_stack_unwind_buffer = (addr, buffer, count) => {
  var stack;
  if (UNWIND_CACHE.last_addr ***REMOVED*** addr) {
    stack = UNWIND_CACHE.last_stack;
  } else {
    stack = jsStackTrace().split("\n");
    if (stack[0] ***REMOVED*** "Error") {
      stack.shift();
    }
...
var getEnvStrings = () => {
  if (!getEnvStrings.strings) {
    ...
    for (var x in ENV) {
      ...
      if (ENV[x] ***REMOVED***= undefined) delete env[x]; else env[x] = ENV[x];
    }
    var strings = [];
    for (var x in env) {
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
  ...
  try {
    ...
    if (stream.getdents && offset ***REMOVED***= 0 && whence ***REMOVED***= 0) stream.getdents = null;
    // reset readdir state
    return 0;
  } catch (e) {
    if (typeof FS ***REMOVED*** "undefined" || !(e.name ***REMOVED***= "ErrnoError")) throw e;
...
var allocate = (slab, allocator) => {
  var ret;
  assert(typeof allocator ***REMOVED*** "number", "allocate no longer takes a type argument");
  assert(typeof slab != "number", "allocate no longer takes a number as arg0");
  if (allocator ***REMOVED*** ALLOC_STACK) {
...
/**
  ...
  var toC = {
    "string": str => {
      var ret = 0;
      if (str !***REMOVED*** null && str !***REMOVED*** undefined && str !***REMOVED*** 0) {
        // null string
  ...
  function convertReturnValue(ret) {
    if (returnType ***REMOVED***= "string") {
      return UTF8ToString(ret);
    }
    if (returnType ***REMOVED***= "boolean") return Boolean(ret);
    return ret;
  }
  var func = getCFunc(ident);
  var cArgs = [];
  var stack = 0;
  assert(returnType !***REMOVED*** "array", 'Return type should not be "array".');
  if (args) {
    for (var i = 0; i < args.length; i++) {
      var converter = toC[argTypes[i]];
      if (converter) {
        if (stack ***REMOVED***= 0) stack = stackSave();
  ...
  var ret = func(...cArgs);
  function onDone(ret) {
    if (stack !***REMOVED*** 0) stackRestore(stack);
...
assert(emval_handles.length ***REMOVED***= 5 * 2);
...
// but before the wasm module is created.
{
  ...
  assert(typeof Module["memoryInitializerPrefixURL"] ***REMOVED*** "undefined", "Module.memoryInitializerPrefixURL option was removed, use Module.locateFile instead");
  assert(typeof Module["pthreadMainPrefixURL"] ***REMOVED*** "undefined", "Module.pthreadMainPrefixURL option was removed, use Module.locateFile instead");
  assert(typeof Module["cdInitializerPrefixURL"] ***REMOVED*** "undefined", "Module.cdInitializerPrefixURL option was removed, use Module.locateFile instead");
  assert(typeof Module["filePackagePrefixURL"] ***REMOVED*** "undefined", "Module.filePackagePrefixURL option was removed, use Module.locateFile instead");
  assert(typeof Module["read"] ***REMOVED*** "undefined", "Module.read option was removed");
  assert(typeof Module["readAsync"] ***REMOVED*** "undefined", "Module.readAsync option was removed (modify readAsync in JS)");
  assert(typeof Module["readBinary"] ***REMOVED*** "undefined", "Module.readBinary option was removed (modify readBinary in JS)");
  assert(typeof Module["setWindowTitle"] ***REMOVED*** "undefined", "Module.setWindowTitle option was removed (modify emscripten_set_window_title in JS)");
  assert(typeof Module["TOTAL_MEMORY"] ***REMOVED*** "undefined", "Module.TOTAL_MEMORY has been renamed Module.INITIAL_MEMORY");
  assert(typeof Module["ENVIRONMENT"] ***REMOVED*** "undefined", "Module.ENVIRONMENT has been deprecated. To force the environment, use the ENVIRONMENT compile-time option (for example, -sENVIRONMENT=web or -sENVIRONMENT=node)");
  assert(typeof Module["STACK_SIZE"] ***REMOVED*** "undefined", "STACK_SIZE can no longer be set at runtime.  Use -sSTACK_SIZE at link time");
  if (Module["preInit"]) {
    if (typeof Module["preInit"] ***REMOVED*** "function") Module["preInit"] = [ Module["preInit"] ];
...
// ***REMOVED***= Auto-generated postamble setup entry stuff ***REMOVED***=
var calledRun;
...
if (typeof exports ***REMOVED***= 'object' && typeof module ***REMOVED***= 'object') {
  module.exports = loadWasmTtsBindings;
  // This default export looks redundant, but it allows TS to import this
  // commonjs style module.
  module.exports.default = loadWasmTtsBindings;
} else if (typeof define ***REMOVED***= 'function' && define['amd'])
...
if (isNode) isPthread = require('node:worker_threads').workerData ***REMOVED***= 'em-pthread'
