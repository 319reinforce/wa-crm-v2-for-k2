...
| `docs/GIT_BRANCH_WORKFLOW.md` | Git 分支规范：`main / dev / feature/* / fix/* / hotfix/*` |
...
1. `main` 仅用于生产环境，可部署，不直接开发。
2. `dev` 是默认集成线；所有新任务默认从 `dev` 拉分支。
3. 新功能使用 `feature/*`，普通 bug 使用 `fix/*`，线上紧急修复使用 `hotfix/*`。
4. `codex/*` 仅作为临时工作分支，不再作为长期协作命名规范。
5. 详细流程见 `docs/GIT_BRANCH_WORKFLOW.md`。
