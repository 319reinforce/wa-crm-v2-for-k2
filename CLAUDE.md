...
5. 再读 SFT_PROJECT.md → 深入理解 SFT 训练和 Experience Router
...
| 文件 | 作用 |
|------|------|
| `server/index.cjs` | Express 服务器，REST API 端口 3000 |
...
> **现状**：`server/routes/experience.js` 已实现，`POST /api/experience/route` 根据 client_id/operator 自动路由到 Beau 或 Yiyun 的专属 AI 体验。
...
| `docs/DOCUMENT_RETENTION_AUDIT_20260427.md` | 当前文档留存/删除清理记录 |
| `docs/archive/PRE_20260420_DOCS_ARCHIVE.md` | 2026-04-20 前旧文档合并归档，不作为当前施工入口 |
...
1. 列出本次完成的三件事
2. 列出剩余未完成事项
3. 说明 Obsidian 同步状态：`synced: <note path>` / `not required: <reason>` / `blocked: <reason>`
4. 告诉用户："要继续吗？"

如果用户说"结束"或"够了"才停止。

WA CRM v2 的记忆同步只使用 Obsidian。新增或更新规范、设计、runbook、PRD、handoff 时，按 `docs/OBSIDIAN_MEMORY_STANDARD.md` 写入 `docs/obsidian/`。
