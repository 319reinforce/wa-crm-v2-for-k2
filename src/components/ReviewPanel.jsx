...
export function ReviewPanel({ modelOutput, onClose, onConfirmed }) {
  ...
  const handleConfirm = async () => {
    const finalOutput = selected === 'opt1'
      ? modelOutput.opt1
      : selected === 'opt2'
        ? modelOutput.opt2
        : customInput.trim()
    ...
    try {
      const sftRecord = {
        ...
        human_selected: selected === 'custom' ? 'custom' : selected,
        human_output: finalOutput,
        diff_analysis: {
          model_predicted: selected === 'opt1' ? modelOutput.opt1 : selected === 'opt2' ? modelOutput.opt2 : null,
          model_rejected: selected === 'opt1' ? modelOutput.opt2 : selected === 'opt2' ? modelOutput.opt1 : null,
  ...
  const isValid = selected === 'opt1' || selected === 'opt2' || (selected === 'custom' && customInput.trim())
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div
            ...
            className={`rounded-xl border-2 transition-all cursor-pointer ${
              selected === 'custom' ? 'border-amber-500 bg-amber-500/10' : 'border-slate-600 bg-slate-800 hover:border-slate-500'
            }`}
          >
              ...
              {selected !== 'custom' && (
                <>
                  <div className="flex items-start gap-2">
                    <span className={selected === 'opt1' ? 'text-blue-400' : 'text-green-400'}>{selected === 'opt1' ? 'A' : 'B'}:</span>
                    <span className="text-slate-400 line-clamp-2 flex-1">{selected === 'opt1' ? modelOutput.opt1 : modelOutput.opt2}</span>
                    <span className="text-emerald-400 shrink-0">✓ 选中</span>
                  </div>
                  <div className="flex items-start gap-2">
                    <span className={selected === 'opt1' ? 'text-green-400' : 'text-blue-400'}>{selected === 'opt1' ? 'B' : 'A'}:</span>
                    <span className="text-slate-600 line-clamp-2 flex-1">{selected === 'opt1' ? modelOutput.opt2 : modelOutput.opt1}</span>
...
function OptionCard({ label, content, selected, onClick, accent = 'blue' }) {
  const borderColor = selected
    ? accent === 'blue' ? 'border-blue-500 bg-blue-500/10' : 'border-green-500 bg-green-500/10'
    : 'border-slate-600 hover:border-slate-500 bg-slate-800'

  return (
    <div onClick={onClick} className={`rounded-xl border-2 transition-all cursor-pointer ${borderColor}`}>
      <div className="px-4 py-3">
        <div className={`text-xs font-semibold mb-2 ${selected ? (accent === 'blue' ? 'text-blue-400' : 'text-green-400') : 'text-slate-400'}`}>
          {selected ? '✅ ' : '○ '}{label}
        </div>
