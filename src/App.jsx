...
function App() {
  ...
  const computeUnread = (messages) => {
    ...
    for (let i = 0; i < msgs.length; i++) {
      if (msgs[i].role === 'user') {
        const hasReply = msgs.slice(i + 1).some(m => m.role === 'me')
        if (!hasReply) unread++
      }
    }
    return unread
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto">
              ...
              <div className="flex items-center gap-1 px-4 py-3 border-b" style={{ borderColor: WA.borderLight }}>
                {['', 'Beau', 'Yiyun'].map(o => (
                  <button key={o} onClick={() => setFilterOwner(o)} className="flex-1 py-2 rounded-xl text-xs font-semibold" style={{ background: filterOwner === o ? WA.teal : 'transparent', color: filterOwner === o ? 'white' : WA.textMuted }}>{o === '' ? '全部' : o}</button>
                ))}
              </div>
              {/* Creator list */}
              <div>
      ...
      <div className="hidden md:flex h-screen overflow-hidden" style={{ background: WA.lightBg }}>
        ...
        <div
          className="w-1.5 shrink-0 cursor-col-resize group relative z-10"
          style={{ background: dragging === 'detail-chat' ? WA.teal : 'transparent' }}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {wacrm.priority && wacrm.priority !== 'low' && (
            <span className="text-xs px-2 py-0.5 rounded-full font-medium" style={{ background: wacrm.priority === 'high' ? 'rgba(239,68,68,0.12)' : 'rgba(245,158,11,0.12)', color: wacrm.priority === 'high' ? '#ef4444' : '#f59e0b' }}>
              {wacrm.priority === 'high' ? '🔴 高' : '🟡 中'}
            </span>
          )}
