...
import useResizablePanelWidths from './hooks/useResizablePanelWidths'
...
import { ContactManagementPage, ManualCreatorModal } from './components/ContactManagementPage'
...
import { isAppAuthAdmin, readAppAuthSnapshot, subscribeAppAuthChanges } from './utils/appAuth'
...
function App() {
  const [authSnapshot, setAuthSnapshot] = useState(() => readAppAuthSnapshot())
  useEffect(() => subscribeAppAuthChanges(setAuthSnapshot), [])
  const lockedOwner = authSnapshot.scopeOwner || ''
  const ownerLocked = !!authSnapshot.ownerLocked && !!lockedOwner
  ...
  const { dragging, panelWidths, startDrag } = useResizablePanelWidths({
    limits: { list: { min: LIST_PANEL_MIN_WIDTH, max: LIST_PANEL_MAX_WIDTH } },
  })
  ...
  const selectCreatorById = useCallback(async (creatorId, options = {}) => {
    ...
    try {
      const detail = await fetchJsonOrThrow(buildCreatorDetailUrl(normalizedCreatorId))
      const previous =
  ...
  const saveManualCreator = useCallback(async () => {
    ...
    try {
      ...
      if (result?.creator?.id) {
        const detail = await fetchJsonOrThrow(buildCreatorDetailUrl(result.creator.id))
  ...
  const saveCreatorBasicInfo = useCallback(async (creatorId, payload) => {
    ...
    if (selectedCreator?.id === creatorId) {
      const detail = await fetchJsonOrThrow(buildCreatorDetailUrl(creatorId))
  ...
  return (
    <div
      ...
      data-owner-locked={ownerLocked ? 'true' : 'false'}
      data-locked-owner={lockedOwner}
    >
      ...
      <div className="hidden md:flex flex-1 min-h-0 overflow-hidden p-4 app-shell">
        <div className="flex-1 min-w-0 flex flex-col gap-3">
          <div className="docs-panel shrink-0 px-5 py-4 flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3" style={{ background: WA.shellPanelStrong }}>
            <div className="w-full xl:flex-1 flex items-center gap-4 min-w-0">
              ...
              <div className="flex-1 min-w-0 flex items-center gap-2 overflow-x-auto docs-scrollbar">
            ...
            <div className="w-full xl:w-auto flex items-center gap-2 shrink-0 overflow-x-auto docs-scrollbar pb-1 xl:pb-0">
              <AuthSessionControls />
              <div
          ...
          <div className="flex-1 min-h-0 flex gap-1.5 overflow-hidden">
            {activeTab === 'contacts' ? (
              <ContactManagementPage
                ...
                onBulkWelcomeTemplateKeyChange={setBulkWelcomeTemplateKey}
                isAdmin={isAdmin}
                transferPreview={ownerTransfer.transferPreview}
                transferLoading={ownerTransfer.transferLoading}
                transferExecuting={ownerTransfer.transferExecuting}
                transferError={ownerTransfer.transferError}
                onLoadTransferPreview={ownerTransfer.loadTransferPreview}
                onExecuteOwnerTransfer={ownerTransfer.executeTransfer}
                onClearTransferPreview={ownerTransfer.clearTransferPreview}
              />
            ) : (
            <>
            <div
            ...
            >
              ...
              <div className="flex-1 overflow-y-auto docs-scrollbar" style={{ background: WA.shellPanel }}>
                ...
                {loading && creators.length === 0 ? (
                  ...
                  <KanbanView columns={KANBAN_COLUMNS} creators={filteredCreators} onCreatorClick={c => handleSelectCreator(c)} />
                )}
              </div>
