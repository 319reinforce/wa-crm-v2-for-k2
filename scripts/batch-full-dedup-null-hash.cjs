...
async function main() {
  ...
  console.log('[batch-full-dedup-null-hash] options=', {
    apply: options.apply,
    owners: options.owners,
    batch_limit: options.batchLimit,
  });
  ...
  for (const owner of options.owners) {
    ...
    for (const creator of creators) {
      ...
      console.log('[batch-full-dedup-null-hash] processed', result);
    }
  }
  ...
  console.log('[batch-full-dedup-null-hash] summary=', report.summary);
  console.log('[batch-full-dedup-null-hash] report_path=', reportPath);

  await closeDb();
}

main().catch(async (error) => {
  console.error('[batch-full-dedup-null-hash] fatal:', error.message);
  await closeDb();
  process.exit(1);
});
