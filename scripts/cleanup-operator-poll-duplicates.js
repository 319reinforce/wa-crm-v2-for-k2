...
const OPERATOR = String(process.env.POLL_DUP_OPERATOR || '').trim();
const WINDOW_START = process.env.POLL_DUP_START || '2026-04-10 00:00:00';
const WINDOW_END = process.env.POLL_DUP_END || null;
const MIN_REPEAT = Number(process.env.POLL_DUP_MIN_REPEAT || 3);

if (!OPERATOR) {
    console.error('POLL_DUP_OPERATOR is required');
    process.exit(1);
}
...
async function main() {
    ...
    const groupParams = WINDOW_END ? [OPERATOR, WINDOW_START, WINDOW_END, MIN_REPEAT] : [OPERATOR, WINDOW_START, MIN_REPEAT];

    const groups = await db.prepare(`
        SELECT creator_id, role, text, COUNT(*) AS cnt, MIN(id) AS keep_id
        FROM wa_messages
        WHERE operator = ?
          AND created_at >= ?
          ${endClause}
        ...
        await db.transaction(async (tx) => {
            for (const group of groups) {
                const rows = await tx.prepare(`
                    ...
                    FROM wa_messages
                    WHERE creator_id = ? AND text = ? AND operator = ? AND created_at >= ? ${WINDOW_END ? 'AND created_at <= ?' : ''}
                    ORDER BY timestamp ASC, id ASC
                `).all(...(WINDOW_END
                    ? [group.creator_id, group.text, OPERATOR, WINDOW_START, WINDOW_END]
                    : [group.creator_id, group.text, OPERATOR, WINDOW_START]));
    ...
    const reportDir = path.join(process.cwd(), 'reports', `${OPERATOR.toLowerCase()}-poll-duplicate-cleanup-${Date.now()}`);
...
main()
    .catch((error) => {
        console.error('[cleanup-operator-poll-duplicates] failed:', error);
        process.exitCode = 1;
    })
    .finally(async () => {
        await closeDb().catch(() => {});
