...
This note records the production rollout boundary for the MySQL optimize branch: API startup can rebuild event-derived snapshots after schema is ready, but production schema changes are not applied automatically by the service process.
