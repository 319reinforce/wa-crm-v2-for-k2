 ...
 */
function buildFullSystemPrompt(clientId, scene, messages) {
    ...
    if (!operator) {
        ...
        return buildBasePrompt(clientInfo, scene);
    }
    ...
    return compileSystemPrompt(operator, scene, clientInfo, clientMemory, policyDocs, exp);
}

/**
 * 编译完整的 system prompt（与 experience.js compileSystemPrompt 逻辑一致）
 */
